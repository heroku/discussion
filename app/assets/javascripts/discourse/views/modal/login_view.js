...
Discourse.LoginView = Discourse.ModalBodyView.extend({
  ...
  **/
  hasAtLeastOneLoginButton: function() {
    return Discourse.SiteSettings.enable_heroku_logins ||
           Discourse.SiteSettings.enable_google_logins ||
  ...
  herokuLogin: function() {
    this.set('authenticate', 'heroku');
    var left = this.get('lastX') - 400;
    var top = this.get('lastY') - 200;
    return window.open(Discourse.getURL("/auth/heroku"), "_blank", "menubar=no,status=no,height=600,width=800,left=" + left + ",top=" + top);
  },
